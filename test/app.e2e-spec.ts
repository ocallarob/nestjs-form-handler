...
describe('AppController (e2e)', () => {
  ...
  const firestoreFormatFormDefinition = Object.keys(
    dummyFormDefinition[formDefinitionServiceName]
  ).reduce(
    (formattedObject, arrayPosition) => ({
      ...formattedObject,
      [arrayPosition]: dummyFormDefinition[formDefinitionServiceName][arrayPosition]
    }),
    {}
  );
  ...
  describe(`Form Definition CRUD`, () => {
    describe(`When POST is made to ${formDefinitionPath}`, () => {
      ...
      describe(`If the entry already exists`, () => {
      ...
      describe.skip(`If the request body is bad`, () => {
        it(`Should return 400`, () => {
          return request(app.getHttpServer())
            .post(`${formDefinitionPath}`)
            .send(badFormDefinition)
            .expect(400);
        });
      });
    });
    ...
    describe(`When GET is made to the ${formDefinitionPath}/${formDefinitionServiceName}`, () => {
      it(`Should return the Form Definition entry from the DB and status 200`, () => {
        return request(app.getHttpServer())
          ...
          .expect(firestoreFormatFormDefinition)
          .expect(200);
      });
      describe(`If the entry is not found`, () => {
        it(`Should return status 404`, () => {
          return request(app.getHttpServer())
            .get(`${formDefinitionPath}/firestoreFormatFormDefinition`)
            .expect(404);
        });
      });
    });
    ...
    describe(`When PUT is made to the ${formDefinitionPath}/${formDefinitionServiceName}`, () => {
      ...
      describe(`If the entry is not found`, () => {
        it(`Should return status 404`, () => {
          return request(app.getHttpServer())
            .put(`${formDefinitionPath}/dummyDefinition`)
            .send(formDefinitionUpdateData)
            .expect(404);
        });
      });
